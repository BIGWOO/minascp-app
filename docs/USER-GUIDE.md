...
偏好設定（⌘,）→「外觀」提供明亮玻璃、深色玻璃與跟隨 macOS。新設定預設明亮玻璃、背景透明度 50%。外觀會立即套用到主視窗、站台管理、連線資訊與對話框，保留既有連線、分頁和選取。

背景透明度 0% 為不透明，100% 為保留原生模糊與可讀性的最高背景透出程度；文字、圖示和選取不會一起變淡。效果會隨後方內容、視窗作用狀態與 macOS 版本改變。設定頁使用示範色彩預覽滑桿效果；實際視窗使用後方桌布／視窗的原生材質。macOS 26 控制群組使用 Liquid Glass，macOS 14／15 使用原生材質替代。右鍵選單保留 macOS 原生呈現，其透明度、陰影與圓角由系統決定，不受背景透明度滑桿控制。

開啟 macOS「減少透明度」時，App 使用實色背景並停用滑桿，保留原值；關閉後恢復。增加對比同樣優先遵循系統設定。「恢復外觀預設值」只重設明暗與透明度。停止調整約 300 毫秒後自動保存，關閉設定或正常離開 App 時補存；外觀保存不啟動傳輸。

- 工具列的上傳／下載隨作用中面板切換。側欄按鈕可收合站台，拖曳兩欄間的分隔線調整寬度。
- 路徑顯示為可點擊的麵包屑；窄視窗將較前面的路徑放進「…」。⌘L 編輯作用中面板的完整路徑，Return 前往，Escape 取消。
- ⌘F 開啟作用中面板的名稱篩選；兩欄各自保存。Escape 或右側清除按鈕清空並關閉篩選，焦點返回檔案表格。
- 傳輸區可收合為進度摘要，失敗／待處理項目會提示展開。新設定預設收合，舊設定保留原本的展開值。

目前本機建置的實際畫面與驗收邊界見 [玻璃外觀驗收](glass-appearance-acceptance-20260911.md)。
